...
def generate_launch_description():
    ...
    robot_description = Command([
        'cat ',
        PathJoinSubstitution([FindPackageShare('ros2_gazebo_object_detection'), 'urdf', 'cam.urdf'])
    ])
    ...
    spawn_camera_entity_node = Node(
        ...
        arguments=[
            '-entity', 'camera',   # Gazebo 上での名前
            '-file', cam_urdf_file      # URDF ファイルパス
        ],
        output='screen'
    )
